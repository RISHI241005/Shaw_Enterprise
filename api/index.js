...
function normalizeProductPayload(input = {}) {
  ...
  if (!name) throw publicError(400, "Product name is required");
  const suppliedPrice = String(input.unitPrice ?? "").trim();
  const parsedPrice = suppliedPrice ? Number(suppliedPrice) : parsePriceAmount(input.price);
  const unitPrice = Number.isFinite(parsedPrice) && parsedPrice > 0 ? money(parsedPrice) : null;
  const suppliedStock = String(input.stockQuantity ?? "").trim();
  const stockQuantity = suppliedStock ? Number(suppliedStock) : 0;
  if (!Number.isSafeInteger(stockQuantity) || stockQuantity < 0 || stockQuantity > 100000) throw publicError(400, "Available packs must be between 0 and 100000");
  return {
    name,
    ...
    packSize: cleanText(input.packSize, 160),
    audience: cleanText(input.audience, 180),
    images: Array.isArray(input.images) ? input.images.filter((image) => typeof image === "string" && image.trim()).slice(0, 12) : [],
...
app.post("/api/admin/products", requireCsrf, asyncRoute(async (req,res) => {
  const p = normalizeProductPayload(req.body);
  await q("INSERT IGNORE INTO product_categories(name,description) VALUES (?,?)", [p.category,`${p.category} products`]);
  const category = await one("SELECT id FROM product_categories WHERE name=?", [p.category]);
  const result = await q("INSERT INTO products(category_id,name,sku,price_label,unit_price,stock_quantity,ordering_enabled,product_type,summary,details,pack_size,audience,featured,status,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?, 'active',?,?)", [category.id,p.name,`SE-${Date.now()}`,p.price,p.unitPrice,p.stockQuantity,p.orderingEnabled,p.productType,p.summary,p.details,p.packSize,p.audience,p.featured,now(),now()]);
  for (const [index,image] of p.images.entries()) await q("INSERT INTO product_images(product_id,image_data,alt_text,sort_order) VALUES (?,?,?,?)", [result.insertId,image,p.name,index]);
  await audit(req,"create","product",result.insertId,p.name); await bump("products");
  res.status(201).json({products:await products(),auditLogs:await q("SELECT * FROM admin_audit_logs ORDER BY id DESC LIMIT 80")});
}));
app.put("/api/admin/products/:id", requireCsrf, asyncRoute(async (req,res) => {
  const id = Number(req.params.id), p = normalizeProductPayload(req.body);
  await q("INSERT IGNORE INTO product_categories(name,description) VALUES (?,?)", [p.category,`${p.category} products`]);
  const category = await one("SELECT id FROM product_categories WHERE name=?", [p.category]);
  await q("UPDATE products SET category_id=?,name=?,price_label=?,unit_price=?,stock_quantity=?,ordering_enabled=?,product_type=?,summary=?,details=?,pack_size=?,audience=?,featured=?,updated_at=? WHERE id=?", [category.id,p.name,p.price,p.unitPrice,p.stockQuantity,p.orderingEnabled,p.productType,p.summary,p.details,p.packSize,p.audience,p.featured,now(),id]);
  await q("DELETE FROM product_images WHERE product_id=?", [id]);
  for (const [index,image] of p.images.entries()) await q("INSERT INTO product_images(product_id,image_data,alt_text,sort_order) VALUES (?,?,?,?)", [id,image,p.name,index]);
  await audit(req,"update","product",id,p.name); await bump("products");
  res.json({products:await products(),auditLogs:await q("SELECT * FROM admin_audit_logs ORDER BY id DESC LIMIT 80")});
}));
...
module.exports = app;
module.exports._test = { escapeHtml, safeEmail, safeContact, normalizeEmail, normalizePhone, publicIdentity, signed, validSigned, poolOptions, generateDummyOtp, parsePriceAmount, money, normalizeOrderItems, calculateOrderTotals, normalizeProductPayload };
