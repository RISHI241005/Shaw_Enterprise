...
const adminAccounts = rows("SELECT * FROM admin_accounts ORDER BY id");
const adminAuthCodes = rows("SELECT * FROM admin_auth_codes ORDER BY id");
...
lines.push("DROP TABLE IF EXISTS admin_auth_codes;");
lines.push("DROP TABLE IF EXISTS admin_accounts;");
...
for (const account of adminAccounts) {
  lines.push(`INSERT INTO admin_accounts (id, username, email, phone, password_hash, email_verified, phone_verified, created_at, updated_at) VALUES (${account.id}, ${esc(account.username)}, ${esc(account.email)}, ${esc(account.phone)}, ${esc(account.password_hash)}, ${account.email_verified ? 1 : 0}, ${account.phone_verified ? 1 : 0}, ${esc(mysqlDate(account.created_at))}, ${esc(mysqlDate(account.updated_at))});`);
}
for (const code of adminAuthCodes) {
  lines.push(`INSERT INTO admin_auth_codes (id, account_id, destination, channel, purpose, code_hash, expires_at, used_at, created_at) VALUES (${code.id}, ${code.account_id || "NULL"}, ${esc(code.destination)}, ${esc(code.channel)}, ${esc(code.purpose)}, ${esc(code.code_hash)}, ${esc(mysqlDate(code.expires_at))}, ${esc(mysqlDate(code.used_at))}, ${esc(mysqlDate(code.created_at))});`);
}
...
lines.push("ALTER TABLE admin_accounts AUTO_INCREMENT = 1001;");
lines.push("ALTER TABLE admin_auth_codes AUTO_INCREMENT = 1001;");
