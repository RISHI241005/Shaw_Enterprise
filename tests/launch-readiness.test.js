...
test("registered administrators are persisted, verified, and can log in through the auth API", async () => {
  const { port, child } = await startServer();
  try {
    const jar = {};
    const loginPage = await request(port, jar, "GET", "/login");
    const csrf = csrfFrom(loginPage.text);
    const registration = await request(port, jar, "POST", "/api/auth/register", {
      username: "api-admin",
    ...
    assert.ok(registration.json.accountId);

    const verification = await request(port, jar, "POST", "/api/auth/verify-signup", {
      accountId: registration.json.accountId,
      emailCode: registration.json.devEmailCode,
      phoneCode: registration.json.devPhoneCode
    }, { "X-CSRF-Token": csrf });
    assert.equal(verification.status, 200);

    const login = await request(port, jar, "POST", "/api/auth/login", {
      username: "api-admin@example.com",
      password: "a-secure-test-password"
    }, { "X-CSRF-Token": csrf });
    assert.equal(login.status, 200);
    assert.equal(login.json.redirect, "/admin");
